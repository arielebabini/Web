...
/**
 * Carica un componente HTML in un container
 * @param {string} containerId - ID del container dove caricare il componente
 * @param {string} componentPath - Percorso del file HTML del componente
 * @returns {Promise} Promise che si risolve quando il componente è caricato
 */
async function loadComponent(containerId, componentPath) {
    ...
    try {
        ...
        // Cerca di caricare il file del componente
        const response = await fetch(componentPath);

        if (!response.ok) {
            console.warn(`⚠️ Component ${componentPath} not found (${response.status})`);
            return false;
        }
